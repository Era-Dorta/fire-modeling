...
void *FireGrid::creator() {
	return new FireGrid;
}

MStatus FireGrid::initialize()
//
//      Descriptions:
...
MStatus FireGrid::compute(const MPlug& plug, MDataBlock& block)
//
//      Descriptions:
...
//
	...
	//
	emit(inPosAry, inVelAry, emitCountPP, dt, speed, inheritFactor, rotatedV,
			fnOutPos, fnOutVel, fnOutTime);
...
void FireGrid::emit(const MVectorArray &inPosAry, // points where new particles from
...
MStatus FireGrid::getWorldPosition(MPoint &point)
//
//      Descriptions:
...
//
	...
	if (!status) {
		status.perror("FireGrid::getWorldPosition: get matrixObject");
		return (status);
	}

	MFnMatrixData worldMatrixData(matrixObject, &status);
	if (!status) {
		status.perror("FireGrid::getWorldPosition: get worldMatrixData");
		return (status);
	}

	MMatrix worldMatrix = worldMatrixData.matrix(&status);
	if (!status) {
		status.perror("FireGrid::getWorldPosition: get worldMatrix");
		return (status);
	}
...
MStatus FireGrid::getWorldPosition(MDataBlock& block, MPoint &point)
//
//      Descriptions:
...
//
	...
	CHECK_MSTATUS(status); //, "ERROR getting hWMatrix from dataBlock.\n");
...
MVector FireGrid::useRotation(MVector &direction) {
	MStatus status;
...
/*void FireGrid::draw( M3dView& view, const MDagPath& path, M3dView::DisplayStyle style, M3dView:: DisplayStatus )
 {
 view.beginGL();
