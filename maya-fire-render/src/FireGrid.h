...
#ifndef SRC_FIREGRID_H_
#define SRC_FIREGRID_H_

#include <maya/MPxEmitterNode.h>

class FireGrid: public MPxEmitterNode {
public:
	FireGrid();
	virtual ~FireGrid();
...
private:
	...
	void emit(const MVectorArray &inPosAry, const MVectorArray &inVelAry,
			...
			double inheritFactor, MVector dirV, MVectorArray &outPos,
			MVectorArray &outVel, MDoubleArray &outTime);
...
// inlines
//
inline double FireGrid::rateValue(MDataBlock& block) {
	MStatus status;
...
inline double FireGrid::speedValue(MDataBlock& block) {
	MStatus status;
...
inline MVector FireGrid::directionVector(MDataBlock& block) {
	MStatus status;
...
inline bool FireGrid::isFullValue(int plugIndex, MDataBlock& block) {
	MStatus status;
	bool value = true;
...
inline double FireGrid::inheritFactorValue(int plugIndex, MDataBlock& block) {
	MStatus status;
	double value = 0.0;
...
inline MTime FireGrid::currentTimeValue(MDataBlock& block) {
	MStatus status;
...
inline MTime FireGrid::startTimeValue(int plugIndex, MDataBlock& block) {
	MStatus status;
	MTime value(0.0);
...
inline MTime FireGrid::deltaTimeValue(int plugIndex, MDataBlock& block) {
	MStatus status;
	MTime value(0.0);
