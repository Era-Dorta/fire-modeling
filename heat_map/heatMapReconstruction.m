...
try
    ...
    if opts.skip_img_preprocessing || opts.use_prev_frame
        ...
        [goal_img, goal_mask, in_img, img_mask, bin_mask_threshold, in_bg_img] = ...
            ...
            opts.bin_mask_threshold,  opts.add_background, true, resize_goal, ...
    ...
    if(opts.use_approx_fitness)
        ...
        if(~(strcmp(opts.solver, 'icm-re') || strcmp(opts.solver, 'icm-re-density') || ...
                strcmp(opts.solver, 'icm')))
            % Estimate the best density for the optimization
            density_folder = fullfile(output_img_folder, 'density-estimates', 'initial');
            [best_density, f_density, d_norm] =  estimate_density_scale_initial( maya_send,  ...
    ...
    switch opts.solver
        ...
        case 'cmaes'
            ...
            heat_map_v = do_cmaes_solve( init_heat_map, ...
                fitness_foo, paths_str, summary_data, goal_img, goal_mask, ...
                opts, maya_send);
        case 'lhs'
