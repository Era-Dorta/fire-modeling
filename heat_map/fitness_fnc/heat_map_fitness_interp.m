...
%HEAT_MAP_FITNESS_INTERPOLATION Heat map fitness function
...
for i=1:length(IMAGES_DB)
    c_distances(i) = norm(IMAGES_DB_HEATMAPS(:,i) - heat_map_v');
end
...
if(length(IMAGES_DB) >= 2 && min_dist < dist_threshold)
    ...
    heat_map_rel = heat_map_v' ./ IMAGES_DB_HEATMAPS(:,min_ind(1));
    ...
    if(freq(1) < length(heat_map_v))
        ...
        for i=1:length(IMAGES_DB)
            if(i ~= min_ind(1))
                c_distances(i) = norm(IMAGES_DB_HEATMAPS(other_idx,i) - heat_map_v(other_idx)');
            else
        ...
        heat_map_rel = heat_map_v(other_idx)' ./ IMAGES_DB_HEATMAPS(other_idx, min_ind(2));
    ...
    if( (freq(1) + freq(2)) >= 0.9 * length(heat_map_v))
        % The interpolation rate is the scale by the percentage of voxels
        % explained by each heatmap
        weights(1) = scale(1) * (freq(1) / length(heat_map_v));
        weights(2) = scale(2) * (1 - freq(1) / length(heat_map_v));
        ...
        for i=1:num_error_foos
            if(sum(new_img(:)) == 0)
                ...
                error(i) = sum(feval(error_foo{i}, goal_img, new_img));
            end
        end
        return;
    end
...
IMAGES_DB{end + 1} = struct('image', c_img, 'error', error, 'filename', ...
    img_path);
IMAGES_DB_HEATMAPS(:, end + 1) = heat_map_v';
