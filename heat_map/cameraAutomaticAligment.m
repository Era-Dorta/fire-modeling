function cameraAutomaticAligment(args_path, ports, logfile)
%CAMERAAUTOMATICALIGMENT Performs a camera alignment
%   CAMERAAUTOMATICALIGMENT(ARGS_PATH, PORTS, LOGFILE)
...
try
    ...
    dir_num = 0;
    while(exist([opts.scene_img_folder 'ca_search_' num2str(dir_num)], 'dir') == 7)
        dir_num = dir_num + 1;
    end
    
    % Create a new folder to store the data
    output_img_folder = [opts.scene_img_folder 'ca_search_' num2str(dir_num) '/'];
    output_img_folder_name = ['ca_search_' num2str(dir_num) '/'];
    ...
    if isequalFncCell(opts.error_foo{1}, {@MSE, @MSEPerceptual, @MSEPerceptualExp})
        resize_goal = true;
    else
        resize_goal = false;
    end
    ...
    init_cam_attr = get_init_cam_attr(opts);
    ...
    norm_names = get_norm_names( 'GoalOri-Cam', '.tif', num_goal);
    ...
    fitness_foo = @(x)camera_align_fitness(x, init_heat_map.xyz,  ...
        init_heat_map.size, error_foo, opts.scene_name, opts.scene_img_folder,  ...
        output_img_folder_name, maya_send{1}, 1, num_goal, prior_fncs, ...
        prior_weights, opts.color_space, opts.use_cache, true);
    
    %% Solver call
    disp('Launching optimization algorithm');
    switch opts.solver
        case 'ga'
            [heat_map_v, ~, ~] = do_genetic_solve( init_cam_attr, ...
                fitness_foo, paths_str, summary_data, goal_img, goal_mask, ...
                opts);
        case 'sa'
            [heat_map_v, ~, ~] = do_simulanneal_solve( init_cam_attr, ...
        ...
        case 'ga-re'
            ...
            [heat_map_v, ~, ~] = do_genetic_solve_resample(...
                init_cam_attr, fitness_foo, paths_str, maya_send, num_goal, ...
                summary_data, goal_img, goal_mask, opts);
        case 'grad'
            [heat_map_v, ~, ~] = do_gradient_solve_cam(init_cam_attr, ...
                fitness_foo, paths_str, summary_data, goal_img, goal_mask, ...
                opts, maya_send);
        case 'cmaes'
        ...
        case 'icm-re'
            [heat_map_v, ~, ~] = do_icm_re_solve( init_heat_map, fitness_foo, ...
                paths_str, summary_data, goal_img, goal_mask, opts, maya_send);
        case 'icm-re-density'
            fitness_foo = @(x, y)heat_map_fitness_par(x, init_heat_map.xyz,  ...
                init_heat_map.size, error_foo, opts.scene_name, opts.scene_img_folder,  ...
                output_img_folder_name, maya_send, num_goal, prior_fncs, ...
