...
We have ran some of the example simulations provided with the software, and the data was rendered with our shaders, as shown in Figure~\ref{fig:pegoraro_data_simulation_overview}.
The data was also rendered using the VisIt volumetric visualization software, that was developed by the Lawrence Livermore National Laboratory~\cite{VisIt}.
Figure\ref{fig:pegoraro_data_simulation_visit} shows the result in VisIt using texture splats, densities are shown using green and temperatures with orange, darker colours implies a higher voxel values.
...
\begin{figure}[htpb]
		...
		~
 		\begin{subfigure}[t]{0.45\textwidth}
                ...
                \label{fig:pegoraro_data_simulation_shader}
        \end{subfigure}%             
