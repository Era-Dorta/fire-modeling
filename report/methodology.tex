...
Define everything from the very basics, tell the reader that if he is familiar with this to skip the section.
Explain it as in a book, but say first that the decisions will be justified later and come from the RTE.

Explain that we have a scene, with a camera and we want to render an image.
We are going to shoot rays.
Talk about the three stages, simple ray marching.
With direct ilummination.
With indirect ilumination.
With refractions.
