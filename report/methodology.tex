...
The amount of photons which are subjected to this process is controlled by a coefficient $\sigma_s$, the radiance carried by the photons is denoted by $L_i(\lxo)$ and follows a distribution defined by the phase function
...
Fresnel equations describe in detail both reflection and refraction effects when a wave moves between media with different refractive indices.
In the computer graphics community is common to approximate the coefficients using Schlick equations~\cite{Schlick1994}.
In combustion phenomena, the fraction of light which is reflect is assumed to be negligible; thus, the refraction angles for the rays can be easily computed using Snell's law
