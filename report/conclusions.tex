...
A captured image $I_f$ has for each pixel the RGB values that were captured by the sensor at that position.
When a sensor is exposed to a spectral signal, the spectrum is collapsed to RGB using the sensitivity curves for each color, shown in Figure~\ref{fig:camera_sensitivity}.
...
where $r$, $g$ and $b$ are the output coefficients, $i$ is the input spectrum, $s_r$, $s_g$ and $s_b$ are the spectral sensitivity curves and $\lambda$ is a wavelength number.
